...
class SETGENERATOR:
    ...
    def _time_feature(self, df):
        # Convert 'Timestamp' (event numbers) to numeric type
        df[COL.TIME.value] = pd.to_numeric(df[COL.TIME.value])

        # Calculate time since previous event within each case
        df[COL.TSP.value] = df[COL.TIME.value] - df.groupby(COL.CASE.value)[COL.TIME.value].shift(1)
        df[COL.TSP.value].fillna(0, inplace=True)

        # Calculate time since case start
        df[COL.TSSC.value] = df[COL.TIME.value] - df.groupby(COL.CASE.value)[COL.TIME.value].transform('min')
        df[COL.TSSC.value].fillna(0, inplace=True)
    ...
    def _op_label(self, df_train, df_test) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    def _crtp_label(self, df_train, df_test) -> Tuple[pd.DataFrame, pd.DataFrame]:
        temp_train = df_train.groupby(COL.CASE.value)[COL.TIME.value].transform('max')
        df_train[COL.LABEL.value] = temp_train - df_train[COL.TIME.value]

        temp_test = df_test.groupby(COL.CASE.value)[COL.TIME.value].transform('max')
        df_test[COL.LABEL.value] = temp_test - df_test[COL.TIME.value]
        return df_train, df_test
    ...
    def _scaler(self, df_train, df_test, column) -> Tuple[pd.DataFrame, pd.DataFrame, StandardScaler]:
        ...
        if column not in df_train.columns:
            raise KeyError(f"Column '{column}' not found in df_train.")
        if column not in df_test.columns:
            raise KeyError(f"Column '{column}' not found in df_test.")
    ...
    def _meta_scrap(self, df_train, df_test):
        ...
        inject_act_list = None  # Adjust as needed based on your data
    ...
    def _prfx_bucket(self, df, num_res, max_length, for_train, inject_act_list=None):
        ...
        if inject_act_list is not None:
            tensor_set = TensorDataset(
                torch.cat(inputs_act, dim=0).squeeze(-1),
                torch.cat(inputs_attr, dim=0),
                torch.cat(inputs_label, dim=0).squeeze(-1),
                torch.cat(inputs_length, dim=0).squeeze(-1),
                torch.cat(meta_ratio, dim=0)
            )
        else:
            tensor_set = TensorDataset(
                torch.cat(inputs_act, dim=0).squeeze(-1),
    ...
    def SetGenerator(self):
        ...
        # Sort tensors by sequence length
        ...
        num_res += 2  # Add time feature dimensions
        num_act += 1  # Account for zero padding
        if self._task == TASK.NAP.value:
            output_dim += 1  # Account for zero padding
